...
def main():
    ...
    # categories=   [ 'Uniqueness','Ornamental','Social','Familiarity']
    categories= ['可意象性指标']
    ...
    # train_folder =os.path.join(r'E:\sv\imageability',tar_train)  # split into training(80%) and validation(20%)
    train_folder = r'E:\work\build-HEB\可意象性指标\图片集'
    ...
    model = models.convnext.ConvNeXt(block_setting,num_classes=5)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    train(model,device,train_loader, validation_loader,epochs)

    torch.save(model, f'E:/work/build-HEB/可意象性指标/{tar_train}_.pth')
