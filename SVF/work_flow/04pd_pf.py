...
def cal_pd(gdf_ss,longitude,latitude,tar_timezone = 'Europe/London'):
    # 定义原点
    origin = Point(0, 0)
    # 定义线段长度
    length = 100
...
roots = []
shp_names = []
for root, dirs, files in os.walk(r'E:\work\sv_j_ran\20241227\pan2fish\fish_shp'):
    for file in files:
...
for i,shp_path in enumerate(tqdm(shp_paths)): 
    ...
    longitude = float(infos[0])
    latitude = float(infos[1])

    gdf_ss = read_shp_ss(shp_path)
    PD = cal_pd(gdf_ss,longitude,latitude)
    PF = cal_pf(gdf_ss)
