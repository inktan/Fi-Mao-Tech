...
for j in range(0, 6):
    ...
    cluster_1_data = df[df['cluster'] == j]
    ...
    ax.set_yticklabels(contributions.columns,fontsize=16)

    # 设置x轴范围
    ax.set_xlim(0, 1)

    # 添加标题和标签
    ax.set_title(f'Contribution of Features in Cluster {j}', fontsize=28)
    ...
    # 设置刻度标签大小
    ax.tick_params(axis='both', which='major', labelsize=16)  # 主刻度
    ax.tick_params(axis='both', which='minor', labelsize=14)  # 次刻度
    # 显示图例
    # ax.legend(loc='upper right')

    # 显示图形
    # plt.show()
    plt.savefig(f'E:\\work\\sv_shushu\\20250423\\plot\\culster_res10\\cluster{j}.png')
