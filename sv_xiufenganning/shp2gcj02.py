...
def wgs84_to_gcj02_coords(lon, lat):
    """使用coord_convert库将WGS84坐标转换为BD09坐标"""
    return transform.wgs2gcj(lon, lat)
...
def transform_geometry(geom):
    ...
    if geom.geom_type == 'Point':
        # 转换单个点
        x, y = geom.x, geom.y
        new_x, new_y = wgs84_to_gcj02_coords(x, y)
        return Point(new_x, new_y)
    elif geom.geom_type in ['LineString', 'LinearRing']:
        # 转换线
        points = list(geom.coords)
        new_points = [wgs84_to_gcj02_coords(x, y) for x, y in points]
...
output_shp = r"e:\work\sv_xiufenganning\road\_network_gcj02.shp"  # 替换为你的输出文件(BD09坐标系)
