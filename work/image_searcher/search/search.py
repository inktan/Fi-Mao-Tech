...
class Search:
    ...
    def rank_images_by_image(self, image_path, n: int = 10):
        images = [self.loader.open_image(image_path)]
        refer_iamge_embeds = self.embedder.embed_images(images)
        image_embeds, image_paths = self.filter_images([])
        if len(image_paths) == 1:
            return [RankedImage(image_path=image_paths[0], score=1)]
        # scores = (torch.matmul(refer_iamge_embeds, image_embeds.t()) * 100).softmax(dim=1).squeeze().numpy().astype(float)
        scores = torch.stack([torch.dot(refer_iamge_embeds.view(-1), image_embeds[i].view(-1)) for i in range(image_embeds.size(0))])
        
        # print(scores)
        best_images = sorted(list(zip(image_paths, scores)), key=lambda x: x[1], reverse=True)[:n]
        return [RankedImage(image_path=path, score=score) for path, score in best_images]

        # similarities = F.cosine_similarity(refer_iamge_embeds, image_embeds.unsqueeze(0), dim=2)  
        # similarities = similarities.squeeze(0)  
        # # print(similarities)
        # best_images = sorted(list(zip(image_paths, similarities)), key=lambda x: x[1], reverse=True)[:n]
        # return [RankedImage(image_path=path, score=score) for path, score in best_images]

    def test(self):
        images01 = [self.loader.open_image(r'e:\ai_项目\embedding-test\img_results03\0_0.6112343072891235.jpg')]
        images02 = [self.loader.open_image(r'e:\ai_项目\embedding-test\img_results03\1_0.3787488043308258.jpg')]

        refer_iamge_embeds01 = self.embedder.embed_images(images01)
        refer_iamge_embeds02 = self.embedder.embed_images(images02)

        # 展平成 1D 张量
        refer_image_embeds01_flat = refer_iamge_embeds01.view(-1)
        refer_image_embeds02_flat = refer_iamge_embeds02.view(-1)
        # 计算点积
        dot_product = torch.dot(refer_image_embeds01_flat, refer_image_embeds02_flat)
