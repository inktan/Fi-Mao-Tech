...
point_data1['公共交通密度'] = 0

for i,point in enumerate(tqdm(point_data2)):
    # if i>20:
    #     break
    if point.is_empty:
        point_data1.at[i, '公共交通密度'] = 0
        continue

    circle = point.buffer(radius)
    _count = gdf[gdf['geometry'].within(circle)]['geometry'].count()
    print(_count)
    point_data1.at[i, '公共交通密度'] = _count/circle.area

    # break

point_data1.to_csv(r'e:\work\sv_gonhoo\0-Zvalue-Totle-fukuoka-city03.csv', index=False)
