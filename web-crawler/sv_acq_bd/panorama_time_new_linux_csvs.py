# -*- coding:utf-8 -*-
"""
基于地标 CSV 的街景采集（多 CSV 版本）。

功能：
1. **保留原脚本的下载与坐标转换逻辑**；
2. **去除按 landmark_name 建一级子文件夹的归类逻辑**，所有图片统一存放在
   每个 CSV 所在目录下的 `sv_pan01` 文件夹中；
3. **支持递归遍历指定根目录下（含嵌套子目录）的所有 CSV 文件并批量下载**；
4. **支持简单断点续传**：
   - 使用一个进度文件记录“已完成到第几个 CSV”； 
   - 脚本重启时会跳过已处理完的 CSV，从未完成的 CSV 开始继续下载；
   - 不追求精确到具体行的断点，满足“区间位置”需求；
5. **仍然使用多进程下载逻辑以提高性能**。
"""
import math
import json
...
def process_row_single_folder(row, folder_out_path: str) -> str:
    """
...
def process_single_csv(
    ...
    *,
    ...
    """
    处理单个 CSV：
    - 在该 CSV 所在目录下创建/复用一个名为 sv_pan01 的文件夹；
    - 使用多进程下载；
...
def main_multi_csv(root_dir: str,
                   num_processes: int = 5,
                   progress_filename: str = "sv_pan01_progress.json",
                   sv_folder_name: str = "sv_pan01") -> None:
    """
...
if __name__ == '__main__':
    ...
    PROGRESS_FILENAME = 'sv_pan26_progress.json'
