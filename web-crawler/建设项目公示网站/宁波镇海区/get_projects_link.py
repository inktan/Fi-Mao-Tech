...
def make_pudong_gov_request(url):
    ...
    for li in li_matches:
        try:
            ...
            # 提取日期
            date_pattern = r'<b style="float: right;" id="su_bright">(.*?)</b>'
            publish_date = re.search(date_pattern, li).group(1)
...
def extract_project_info(url, project_dir, output_file):
    ...
    # 提取基本信息
    content_div = soup.find(class_="bt-box-main bt-art-0001-D6 clearfix")
    if not content_div:
        print("未找到class='bt-box-main bt-art-0001-D6 clearfix'的标签")
        return False
    ...
    for img in content_div.find_all('img', src=True):
        src = img['src']
        file_url = r'https://www.zh.gov.cn/' + src
        try:
            # 获取文件
...
# 使用示例
base_output_dir = f"Y:\\GOA-项目公示数据\\建设项目公示信息\\宁波\\镇海区\\未分类项目"

# 使用示例
# 规划编制批前公示
number_of_pages = 5
for page in range(number_of_pages):
    url = f'https://www.zh.gov.cn/col/col1229034576/index.html?uid=8913657&pageNum={page+1}'
