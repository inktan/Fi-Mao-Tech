...
with open(json_file_path, "r", encoding="utf-8") as file:
    ...
    print(params_data['梅吟雪飞凤ARC']['fakeid'])
...
params = {
    ...
    "count": "5",
    "query": "",
    "fakeid": params_data['梅吟雪飞凤ARC']['fakeid'],
    "type": "101_1",
    "free_publish_type": "1",
    "sub_action": "list_ex",
...
for i in range(0, masssend_count, 5):
    ...
    for j in range(len(publish_list)):
        ...
        # 创建附图文件夹
        base_output_dir = r'Y:\GOA-项目公示数据\公众号\梅吟雪飞凤ARC'
        safe_dirname = create_safe_dirname(title, publish_date)
        
        illegal_chars = [" ", ".", "/", "/", "\\", "|", '"', ':', '*', '?', '<', '>', '|', '\t']
        for char in illegal_chars:
            safe_dirname = safe_dirname.replace(char, "_")
