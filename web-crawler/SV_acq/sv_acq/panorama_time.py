...
def merge_image(result_cache_path, id,lng,lat,x_count,y_count,save_file_path,timeLine):
    ...
    name = save_file_path + '/' +'sv' +'/' + str(id)+'_' +str(lng)+'_' +str(lat)+ '_' +timeLine+ '.jpg'
    img2.save(name)
...
# 3 4 4*2 = 32
    ...
    with open(csv_path, 'r') as csv_file:  
        ...
        for row in csv_reader:  
            # 打印行内容  
            id_lst.append(row[0])
            lng_lst.append(row[2])
            lat_lst.append(row[3])
    #储存街景的文件夹位置
    temp_path = folder_out_path + '/sv'
    ...
    for i in tqdm(range(len(id_lst))):
        ...
        try:
            ...
            # 索引0 第一项为最近时间点街景
            count = 0
            while (count < len(timeLineIds)):
                try:
                    result_cache_path = temp_path + '/'+str(id)+'_'+ timeLineIds[count]['ID'] +'_'+ timeLineIds[count]['TimeLine']
                    if os.path.exists(result_cache_path) == False:
                        os.makedirs(result_cache_path)
                    get_streetview(result_cache_path ,timeLineIds[count]['ID'] ,x_count,y_count)
                    merge_image(result_cache_path, id,lng,lat,x_count,y_count,folder_out_path,timeLineIds[count]['TimeLine'])
                    break
...
if __name__ == '__main__':
    # 文件夹路径
    csv_path = r'e:\work\Points_Beijing\Points_Beijing_.csv' # 需要爬取的点
    folder_out_path = r'e:\work\Points_Beijing\sv_beijing' # 保存街景文件
