...
# 3 4 4*2 = 32
    ...
    for i in tqdm(range(len(id_lst))):
        ...
        try:
            ...
            # 索引0 指定某一个年份
            for i in range(len(timeLineIds)):

                if int(timeLineIds[i]['TimeLine'][0:4]) <= 202000000:
                    result_cache_path = temp_path + '/'+str(id)+'_'+ timeLineIds[i]['ID'] +'_'+ timeLineIds[i]['TimeLine']
                    if os.path.exists(result_cache_path) == False:
                        os.makedirs(result_cache_path)
                    get_streetview(result_cache_path ,timeLineIds[0]['ID'] ,x_count,y_count)
                    merge_image(result_cache_path, id,lng,lat,x_count,y_count,folder_out_path,timeLineIds[i]['TimeLine'])
                    break
...
if __name__ == '__main__':
    # 文件夹路径
    csv_path = r'f:\sv_yanxiandian\yanxiandian.csv' # 需要爬取的点
    folder_out_path = r'f:\sv_yanxiandian\sv_yanxiandian' # 保存街景文件
