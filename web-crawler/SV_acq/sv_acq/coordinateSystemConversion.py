...
def merge_image(result_cache_path, id,lng,lat,x_count,y_count,save_file_path,timeLine):
    ...
    #name = save_file_path + '/' +'sv' +'/' + str(id)+'_' +str(lng)+'_' +str(lat)+ '.jpg'
    img2.save(name)
    ...
    for j in tqdm(range(len(id_lst))):
        ...
        try:
            ...
            for i in range(len(filtered_panoramas)):
                ...
                merge_image(result_cache_path, id,lng,lat,x_count,y_count,folder_out_path,timeLine)
                break
...
if __name__ == '__main__':
    ...
    folder_out_path = r'e:\work\sv_cynthia\sv_points' # 保存街景文件
