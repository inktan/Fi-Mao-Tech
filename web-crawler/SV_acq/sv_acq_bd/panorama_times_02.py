...
def main(csv_path,folder_out_path):
    ...
    for i in tqdm(range(len(id_lst))):
        ...
        try:
            ...
            #     nested_timeLineIds =[float(lng),float(lat)]
            #     nested_timeLineIds.extend(timeLine['TimeLine'] for timeLine in timeLineIds)
            #     nested_list.append(nested_timeLineIds)
            #     print(nested_timeLineIds)
            #     continue

            # 每个地点单独存一个文件夹，使用id命名
            pic_path = folder_out_path
            # +'/'+str(id)+'_' +str(lng)+'_' +str(lat)
            if os.path.exists(pic_path) == False:
                os.mkdir(pic_path)
                
            for timeLine in timeLineIds:
                # if '2020' not in timeLine['TimeLine']:
                #     continue
                result_cache_path = temp_path + '/'+str(id)+'_'+ timeLine['ID'] +'_'+ timeLine['TimeLine']
                if os.path.exists(result_cache_path) == False:
                    os.mkdir(result_cache_path)
