...
def process_geometry(row,interval):
    ...
    return [(row['road_id'], point.x, point.y) for point in points]

# 从google map量去的坐标为gcj-2，需要转为wgs84
coords = [
   (120.29594285169537, 22.640380898522057),
   (120.3020596591006, 22.640272652562786),
   (120.3017980257455, 22.637574802654363),
   (120.2956000217815, 22.63756647587511),
]
...
# 创建GeoDataFrame
gdf = gpd.GeoDataFrame(geometry=[Polygon(wgs_coords)], crs="EPSG:4326")
# 转换为UTM投影（自动选择最佳UTM区域）
utm_gdf = gdf.to_crs(gdf.estimate_utm_crs())
# 执行缓冲操作
buffered = utm_gdf.buffer(1000)
# 转换回WGS84
buffered_wgs = buffered.to_crs("EPSG:4326")
# 获取缓冲后的多边形
buffered_polygon = buffered_wgs.geometry[0]

# 为指定的多边形区域下载路网数据
# G = ox.graph_from_polygon(polygon, network_type='drive')
# G = ox.graph_from_polygon(polygon, network_type='all')
G = ox.graph_from_polygon(buffered_polygon, network_type='all')

# 将图转换为 GeoDataFrame
gdf = ox.graph_to_gdfs(G, nodes=False)
gdf["road_id"] = range(len(gdf))  # 新增 0,1,2... 列

# 定义保存 SHP 文件的文件路径
shp_file_path = r"E:\work\sv_zhangzhikai\_network.shp"
...
points_df = pd.DataFrame(list(results), columns=['road_id', 'longitude', 'latitude'])
