...
query_text="Please provide a detailed description of the trees in the image, from left to right, in JSON format. For example, {'position ':' left01 ',' SpecialsNam ':' CommonName '.}"
# 在这里配置您在本站的API_KEY
...
def main(img_folder):
    ...
    # img_paths =[r'e:\work\sv_pangpang\test\tree_out\FID_15_2024-09_pano.png']
    ...
    for i, img_path in enumerate(tqdm(img_paths)):
        ...
        if '.jpg' in img_path:
            txt_path = img_path.replace('.jpg','.txt').replace('tree_out_ai','tree_out_ai_chat')
        elif '.png' in img_path:
            txt_path = img_path.replace('.png','.txt').replace('tree_out_ai','tree_out_ai_chat')
        elif '.JPG' in img_path:
            txt_path = img_path.replace('.JPG','.txt').replace('tree_out_ai','tree_out_ai_chat')
        elif '.jpeg' in img_path:
            txt_path = img_path.replace('.jpeg','.txt').replace('tree_out_ai','tree_out_ai_chat')
...
if __name__ == '__main__':
    img_folder = r'E:\work\sv_pangpang\test\tree_out_ai'
    main(img_folder)