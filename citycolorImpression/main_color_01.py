...
def cityColorThemes(imgPaths,imgNames):
    for i, img_path in enumerate(tqdm(imgPaths)):
        ...
        #     quantize_ratios[tuple(quantize[j])]=ratios[j]
        # 将排序后的列表转换回字典
        # quantize_ratios = dict(sorted(quantize_ratios.items(), key=lambda d: d[1], reverse=False))
        # quantize=quantize_ratios.keys()
        # ratios = quantize_ratios.values()

        # show_cityColor_impression(img_path,imgNames[i],quantize,ratios)
        ...
        with open(r'F:\color_daidonggua\color_cal_.csv','a' ,newline='') as f:
            writer = csv.writer(f)
            try:
...
def show_cityColor_impression(image_path,imgName,quantize,ratios):
    fig = plt.figure(figsize=(15, 9))
    grid = plt.GridSpec(1, 2, hspace=0, wspace=0,width_ratios=[9,3])
    ...
    ax4.set_anchor('W')
    # index = imgName.rfind('.')
    image_path= image_path.replace('sv_tree','sv_tree_out')
...
if __name__ == "__main__":
    ...
    # 文件夹路径
    folder_path = r'F:\color_daidonggua\images' # 需要分析的文件夹路径
    # folder_out_path = r'F:\plant_color\sv_tree_out' # 保存文件结果的文件夹路径
    ...
    with open(r'F:\color_daidonggua\color_cal_.csv','w' ,newline='') as f:
        writer = csv.writer(f)
        ...
        #                 ])
        writer.writerow(["File_Name",\
                    "Color_tone_1","Color_tone_2","Color_tone_3",\
