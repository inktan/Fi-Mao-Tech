...
def cityColorThemes(imgPaths,imgNames,folder_out_path):
    for i, img_path in enumerate(tqdm(imgPaths)):
        ...
        show_cityColor_impression(img_path,imgNames[i],quantize,ratios,counts,folder_out_path)
        ...
        with open(r'f:\sv\街景-sv\color_cal_.csv','a' ,newline='') as f:
            writer = csv.writer(f)
            try:
...
def show_cityColor_impression(image_path,imgName,quantize,ratios,counts,folder_out_path):
...
if __name__ == "__main__":
    ...
    # with open(r'f:\sv\街景-sv\color_cal_.csv','w' ,newline='') as f:
    #     writer = csv.writer(f)
